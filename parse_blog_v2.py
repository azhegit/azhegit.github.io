...
def read_yaml(openfile, categ, tag, filename):
    ...
    try:
        data = {} if len(md_context[0]) == 0 else yaml.safe_load(''.join(md_context[0][1:-1]))
    except:
        print(openfile + '修改失败！')
    # data = {}
    # print(filename)
    # print(filename.replace('.md', ''))
    data['title'] = filename.replace('.md', '').decode('utf-8')
    # categories = [] if not data.has_key('categories') else data['categories']
    tags = [] if not data.has_key('tags') else data['tags']
    ...
    if data.has_key('thumbnailImage'):
    ...
    if not data.has_key('date'):
        data['date'] = time.strftime('%Y-%m-%d %H:%M:%S+08:00',
                                     time.localtime(os.stat(openfile).st_birthtime))
    elif tag == 'welcome':
    ...
    with open(openfile, 'w') as f:
        context = str(yaml.dump(data).decode('unicode_escape'))
        f.write('---\n' + context + '---\n' + md_context[1])
